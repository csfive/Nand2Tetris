...
2. `PARTS` 定义内部结构
3. 连线用 `引脚名=信号名`
4. <mark>切片用 `[start..end]`</mark>
5. <mark>两个特殊常量 `true/false`</mark>
6. 注释用 `//` 或 `/*...*/`
...
1. Each chip can be implemented in more than one way. The simpler the implementation, the
   better. As a general rule, <mark>strive to use as few chip-parts as possible</mark>.
2. Although <mark>each chip can be implemented directly from Nand gates only</mark>, you can use any other
